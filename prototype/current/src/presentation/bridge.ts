...
export class PresentationBridge {
  ...
  sync(snapshot: Snapshot) {
    this.entityCache = new Map(
      snapshot.entities.map((e) => [e.id, { ...e, status: { ...e.status } }])
    );
  }
    ...
    for (const e of events) {
      if (e.type === 'SkillActivated')
        ...
        const dx = e.x - e.sourceX,
          ...
          dirX = dx / mag,
          dirZ = dz / mag;
        const maxHp = Math.max(1, target?.maxHp ?? e.amount),
          ratio = e.amount / maxHp;
        const intensity = clamp(
          0.28 + Math.sqrt(Math.max(0, ratio)) * 1.35 + (e.crit ? 0.28 : 0),
          0.32,
          1.65
        );
        ...
        const actor =
          this.entityCache.get(e.entity) ?? postById.get(e.entity) ?? fallbackDeadActor(e);
