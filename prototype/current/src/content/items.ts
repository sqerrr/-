import type { ItemCategory, ItemId } from '../core/types.js';

/**
 * What an item does to whoever carries it. Keeping the effect in data rather than in a branch
 * per item is what lets one apply routine serve twenty items, and it is the same shape the
 * phenomena will move to in step 3.
 */
export type ItemEffect =
  | { kind: 'armor'; amount: number }
  | { kind: 'maxHp'; amount: number }
  | { kind: 'barrierOnEliteKill'; amount: number }
  | { kind: 'damageTakenMul'; amount: number }
  | { kind: 'damageMul'; amount: number }
  | { kind: 'crit'; amount: number }
  | { kind: 'siphon'; amount: number }
  | { kind: 'eliteDamageMul'; amount: number }
  | { kind: 'moveSpeedMul'; amount: number }
  | { kind: 'tempo'; amount: number }
  | { kind: 'dashCooldownMul'; amount: number }
  | { kind: 'dashIFrameMul'; amount: number }
  | { kind: 'pickupRadiusMul'; amount: number }
  | { kind: 'fortune'; amount: number }
  | { kind: 'xpMul'; amount: number }
  | { kind: 'relicRateMul'; amount: number }
  | { kind: 'coreBonus'; amount: number }
  | { kind: 'refusalDamageMul'; amount: number };
