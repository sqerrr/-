...
function setup(build: Build, seed: number, affix: EliteAffix): {sim:any; elite:any} {
  const sim:any = new Simulation({seed,hz,runDuration:480,benchmark:true,mode:'clean'});
...
function drive(sim:any, elite:any, build:Build, frame:number) {
  ...
  const mm=Math.hypot(moveX,moveZ)||1; moveX/=mm; moveZ/=mm;
...
function run(build:Build, seed:number, affix:EliteAffix):Result {
  ...
  for(let i=0;i<maxFrames && elite.hp>0;i++) {
    ...
    for(const ev of sim.events) if(ev.type==='RareEvent' && ev.title==='ЩИТ СЛОМАН') shieldBreaks++;
  }
...
const affixes:EliteAffix[]=['none','shielded'];
const rows:Result[]=[];
for(const affix of affixes) for(const build of buildDefs) for(const seed of seeds) rows.push(run(build,seed,affix));
const median=(xs:number[])=>{const a=[...xs].sort((x,y)=>x-y),h=a.length>>1;return a.length%2?a[h]:(a[h-1]+a[h])/2};
...
for(const affix of affixes){
  ...
  for(const build of buildDefs){
    ...
    for(const x of q)for(const [k,v] of Object.entries(x.damageBySource))merged[k]=(merged[k]??0)+v;
...
if(process.argv.includes('--assert')){
  ...
  const med=(name:string)=>median(open.filter(x=>x.build===name).map(x=>x.seconds));
