...
for (const id of expectedSkills) {
  ...
  const s = skills[id as keyof typeof skills];
  if (!s) fail(`missing Skill definition ${id}`);
...
if (['rail_spear', 'toxic_mist', 'repulse_halo'].some((id) => skillOrder.includes(id as any)))
