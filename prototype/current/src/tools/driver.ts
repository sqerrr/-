 ...
 */
export function pickOffer(s: Snapshot): number {
 ...
 */
export function steer(s: Snapshot, tick: number, hz: number) {
  const a = tick / (hz * 4.3);
  const sx = Math.cos(a) * 0.65,
    sy = Math.sin(a * 0.73) * 0.58;
  let moveX = (sx + sy) * 0.7071,
    moveZ = (-sx + sy) * 0.7071;
  let bestX = 0,
    bestZ = 0,
    bestD = 26,
  ...
  if (found) {
    const dx = bestX - s.player.x,
      dz = bestZ - s.player.z,
      m = Math.hypot(dx, dz) || 1;
    moveX = dx / m;
    moveZ = dz / m;
  }
  const targets = [...s.entities].sort(
    (p, q) =>
