...
function targetOf(s: Snapshot) {
  return [...s.entities].sort(
    (a, b) =>
...
function control(s: Snapshot, strategy: Strategy, t: number): Command {
  ...
  if (e) {
    ...
    const sx = -az * side,
      sz = ax * side;
    const desired =
      strategy === 'close' ? 2.25 : strategy === 'field' ? 4.6 : strategy === 'ranged' ? 9.0 : 5.8;
    let radial = 0;
    if (d > desired + 1.1) radial = 1;
    else if (d < desired - 0.9) radial = -1;
...
function run(build: Build, seed: number, duration = 240): RunResult {
  ...
  for (let i = 0; i < maxTicks; i++) {
    ...
    if (sim.snapshot().player.hp <= 0 || sim.snapshot().finished) break;
  }
  const s = sim.snapshot(),
    tel = sim.telemetry();
  return {
...
const all: RunResult[] = [];
for (const b of builds) for (const seed of seeds) all.push(run(b, seed));
const summary = builds.map((b) => {
  const rs = all.filter((x) => x.build === b.id);
  const merged: Record<string, number> = {};
  for (const r of rs)
    for (const [k, v] of Object.entries(r.damageBySource)) merged[k] = (merged[k] ?? 0) + v;
  const total = Object.values(merged).reduce((a, v) => a + v, 0) || 1;
  const contrib = Object.entries(merged)
    .map(([k, v]) => [k, v / total] as const)
    .sort((a, b) => b[1] - a[1]);
  const activeContrib = b.loadout.slots.map((id) => [id, (merged[id] ?? 0) / total] as const);
  return {
    id: b.id,
    name: b.name,
    ...
    shotHitRate: +(
      rs.reduce((n, r) => n + r.shotHits, 0) /
      Math.max(
        1,
        rs.reduce((n, r) => n + r.shots, 0)
      )
    ).toFixed(3),
...
const dmg = summary.map((x) => x.damageMean),
  kill = summary.map((x) => x.killsMean);
const ratio = Math.max(...dmg) / Math.max(1, Math.min(...dmg)),
  killRatio = Math.max(...kill) / Math.max(1, Math.min(...kill));
...
if (process.argv.includes('--assert')) {
  ...
  if (ratio < 1.08) throw new Error('build identities collapsed: damage spread too narrow');
  if (ratio > 4.5) throw new Error('build viability spread too wide');
}
