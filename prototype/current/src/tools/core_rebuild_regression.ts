...
function assert(c: unknown, m: string): asserts c {
  if (!c) throw new Error('core rebuild regression: ' + m);
}
const run = (slots: any[]) => {
  const sim = new Simulation({ seed: 9191, hz: 60, mode: 'clean', benchmark: true });
  sim.configureBenchmarkLoadout({ slots, catalysts: [] });
...
assert(
  ratio > 3.5 && ratio < 4.5,
  `fixed-cycle invariant failed: 1=${one}, 4=${four}, ratio=${ratio}`
);
const s = new Simulation({ seed: 1, hz: 60, mode: 'showcase' }).snapshot();
assert(
  s.chain.catalystRuntime.every((c) => Object.keys(c).length === 1 && 'id' in c),
  'Catalysts must not expose level/potency runtime state'
);
assert(
