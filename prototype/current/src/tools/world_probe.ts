...
const rows = new Map<
  number,
  { enemies: number[]; elites: number[]; shots: number[]; hp: number[]; kills: number[] }
>(marks.map((m) => [m, { enemies: [], elites: [], shots: [], hp: [], kills: [] }]));
for (const seed of seeds) {
  ...
  for (let i = 0; i < 480 * 30; i++) {
    ...
    if (n.player.hp <= 0 || n.finished) break;
  }
}
...
if (process.argv.includes('--assert')) {
  ...
  if (s15.enemies > 18) throw new Error(`opening too dense: ${s15.enemies}`);
