...
function assert(c:unknown,m:string):asserts c{if(!c)throw new Error('planning regression: '+m)}
...
const mortar=s.skills.find(x=>x.id==='mortar_bloom')!;assert(mortar.cold===0,'Planning swap must not impose Cold');
let sawMortar=false;for(let i=0;i<300;i++){sim.step({moveX:0,moveZ:0,aimX:1,aimZ:0});if(sim.events.some(e=>e.type==='SkillActivated'&&e.skill==='mortar_bloom')){sawMortar=true;break}}
assert(sawMortar,'swapped-in Phenomenon should participate immediately');
assert(sim.swapCatalystLocations('reserve',0,'active',0),'Archive catalyst swap rejected');s=sim.snapshot();assert(s.chain.catalysts[0]==='relay','Relay not moved active');assert(s.chain.catalystReserve[0]==='anchor','old active Catalyst not moved to Archive');
