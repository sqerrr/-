...
function assert(cond:unknown,msg:string):asserts cond{if(!cond)throw new Error(`choice regression: ${msg}`)}
const sim=new Simulation({seed:12345,hz:60});
(sim as any).php=1_000_000;(sim as any).maxHp=1_000_000;
let resolved=0, chained=0;
for(let guard=0;guard<60*60*8&&!sim.finished;guard++){
  if(sim.hasChoice){
    const before=sim.snapshot();
    const serial=before.choiceSerial;
    let ok=false;
    if(before.mutationOffer)ok=sim.chooseMutation(0);
    else if(before.rewardOffers)ok=sim.chooseReward(0);
