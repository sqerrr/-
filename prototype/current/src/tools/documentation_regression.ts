...
assert(root.includes('v0.12'),'root entry point is not current version');
assert(root.includes('37_CATALYST_2_0_CHOREOGRAPHY_2026-09-22.md'),'root entry point does not lead to Catalyst 2.0 authority');
...
assert(manifest.includes('v0.12 current'),'manifest is stale');
...
assert(local.includes('v0.12'),'prototype README is stale');
