...
function assert(ok: unknown, message: string): asserts ok {
  if (!ok) throw new Error('squad-regression: ' + message);
}

// LOS is physical cover, not a cosmetic field-only check.
const losSim = new Simulation({ seed: 31337, hz: 60, benchmark: true }) as any;
...
assert(
  losSim.lineOfSight(leftX, rock.z + rock.radius + 1.2, rightX, rock.z + rock.radius + 1.2),
  'clear parallel lane was incorrectly blocked'
);
...
for (let i = 0; i < 300; i++) {
  ...
  overlap = Math.max(
    overlap,
    rock.radius + walker.radius * 0.7 - Math.hypot(walker.x - rock.x, walker.z - rock.z)
  );
}
...
const taskSim = new Simulation({ seed: 20260919, hz: 60, benchmark: true }) as any;
taskSim.ents = [];
...
const tasks = new Set(
  taskSim.ents
    .map((e: any) => (e.squadUntil > taskSim.time ? e.squadTask : 'none'))
    .filter((x: string) => x !== 'none')
);
for (const task of ['press', 'flank', 'intercept', 'hold'])
  assert(tasks.has(task), `missing squad task ${task}`);
const targets = new Set(
  taskSim.ents
    .filter((e: any) => e.squadUntil > taskSim.time)
    .map((e: any) => `${e.orderX.toFixed(1)}:${e.orderZ.toFixed(1)}`)
);
assert(targets.size >= 4, 'squad tasks collapsed into one AoE-friendly destination');
const snapshotTasks = new Set(taskSim.snapshot().entities.map((e: any) => e.squadTask));
