...
export class Simulation {
  ...
  private spawnGuardian(p: Poi) {
    const map: Record<PoiKind, [EliteChassis, EliteAffix]> = {
  ...
  private rollEliteAffix(): EliteAffix {
    ...
    if (r < 0.28) return 'shielded';
    if (r < 0.53) return 'vanguard';
    if (r < 0.77) return 'brood';
    return 'temporal';
  }
  private spawnElite() {
  ...
  private updateEnemyAI() {
    ...
    for (const e of this.ents) {
      ...
      if (e.adaptation === 'intercept') speed *= 1.22;
      if (e.adaptation === 'anchored') speed *= 0.9;
      ...
      if (e.kind === 'elite' && (e.affix === 'shielded' || e.adaptation === 'screening')) {
        ...
        e.shieldAngle += Math.max(
          -(e.adaptation === 'screening' ? 0.62 : 0.82) * dt,
          Math.min((e.adaptation === 'screening' ? 0.62 : 0.82) * dt, diff)
        );
      }
  ...
  private updateEliteAdaptation(e: Ent, d: number, nx: number, nz: number) {
    ...
    if (e.adaptation === 'intercept') {
      ...
      if (e.adaptStage === 0 && e.adaptCooldown <= 0) {
        const tx = this.px + this.playerVX * 0.62,
          tz = this.pz + this.playerVZ * 0.62,
          dx = tx - e.x,
          dz = tz - e.z,
          m = Math.hypot(dx, dz) || 1;
        e.lockedX = dx / m;
  ...
  private spawnShot(e: Ent, nx: number, nz: number) {
    if (this.projectiles.length >= 14) return;
    const speed = 5.0;
  ...
  private updateProjectiles() {
    ...
    const guard =
      this.skillsRuntime.get('orbit_blades')?.mutation === 'orbit_guard' &&
    ...
    for (const c of this.constructs) {
      ...
      if (st && c.cooldown <= 0) {
        ...
        let targets = this.ents.filter(
          (e) => e.hp > 0 && this.targetVisible(e) && Math.hypot(e.x - c.x, e.z - c.z) <= c.range
        );
  ...
  private levelGeometry(_st: SkillRuntime) {
    return 1;
  }
  ...
  private dispatchSkill(id: SkillId, st: SkillRuntime, slot: number) {
    if (id === 'ember_lance') this.castEmber(st, slot);
    else if (id === 'frost_ring') this.castFrost(st, slot);
    else if (id === 'rail_spear') this.castRail(st, slot);
    else if (id === 'cleaver') this.castCleaver(st, slot);
    else if (id === 'chain_arc') this.castArc(st, slot);
    else if (id === 'orbit_blades') this.castOrbit(st, slot);
    else if (id === 'mortar_bloom') this.castMortar(st, slot);
    else if (id === 'sentry') this.castSentry(st, slot);
    else if (id === 'toxic_mist') this.castToxic(st, slot);
    else if (id === 'repulse_halo') this.castRepulse(st, slot);
    else if (id === 'mass_driver') this.castMassDriver(st, slot);
  }
  ...
  private rayHits(ax: number, az: number, range: number, width: number, maxHits = 99) {
  ...
  private rotatedAim(rad: number) {
    const c = Math.cos(rad),
      s = Math.sin(rad);
    return { x: this.aimX * c - this.aimZ * s, z: this.aimX * s + this.aimZ * c };
  }
  private targetVisible(e: Ent) {
    for (const f of this.fields) {
      if (f.kind !== 'veil') continue;
      const inside = Math.hypot(e.x - f.x, e.z - f.z) < f.radius,
        playerInside = Math.hypot(this.px - f.x, this.pz - f.z) < f.radius;
      if (inside && !playerInside && Math.hypot(e.x - this.px, e.z - this.pz) > 3.6) return false;
    }
    return true;
  }
  ...
  private aimPoint(range: number) {
    let best: Ent | undefined,
      bestScore = 999;
    for (const e of this.ents) {
      if (e.hp <= 0 || !this.targetVisible(e)) continue;
      const dx = e.x - this.px,
        dz = e.z - this.pz,
        d = Math.hypot(dx, dz);
      ...
      const lateral = Math.abs(dx * this.aimZ - dz * this.aimX),
    ...
    return best
      ? { x: best.x, z: best.z }
      : { x: this.px + this.aimX * range * 0.72, z: this.pz + this.aimZ * range * 0.72 };
  }
  private combatShape(
    source: string,
    shape: CombatShape,
  ...
  private castEmber(st: SkillRuntime, slot: number) {
    const mut = st.mutation,
    ...
    if (mut === 'ember_furnace') {
      this.fields.push({
        id: this.nextId++,
        x: this.px + this.aimX * 3.3,
        z: this.pz + this.aimZ * 3.3,
  ...
  private castFrost(st: SkillRuntime, slot: number) {
    const mut = st.mutation,
    ...
    this.combatShape(
      'frost_ring',
      { kind: 'circle', x: this.px, z: this.pz, radius: r },
      'control'
    );
    for (const e of this.ents) {
      const d = Math.hypot(e.x - this.px, e.z - this.pz);
  ...
  private castRail(st: SkillRuntime, slot: number) {
    const mut = st.mutation;
  ...
  private castCleaver(st: SkillRuntime, slot: number, repeat = false) {
    const mut = st.mutation,
  ...
  private castArc(st: SkillRuntime, slot: number) {
    ...
    const available = this.ents.filter(
      (e) =>
        e.hp > 0 &&
        this.targetVisible(e) &&
        Math.hypot(e.x - this.px, e.z - this.pz) < this.skillRange(st, skills.chain_arc.baseRange)
    );
  ...
  private castOrbit(st: SkillRuntime, slot: number) {
  ...
  private castMortar(st: SkillRuntime, slot: number) {
    const mut = st.mutation,
      range = this.skillRange(st, skills.mortar_bloom.baseRange),
      p = this.aimPoint(range);
  ...
  private castSentry(st: SkillRuntime, slot: number) {
  ...
  private castToxic(st: SkillRuntime, slot: number) {
    ...
    const x = st.mutation === 'toxic_plume' ? this.px - this.playerVX * 0.55 : this.px,
      z = st.mutation === 'toxic_plume' ? this.pz - this.playerVZ * 0.55 : this.pz;
  ...
  private castRepulse(st: SkillRuntime, slot: number) {
    const mut = st.mutation,
  ...
  private castMassDriver(st: SkillRuntime, slot: number) {
    const mut = st.mutation,
    ...
    const hits = this.rayHits(this.aimX, this.aimZ, range, width, 18);
    let cargo = 0;
    if (mut === 'mass_cargo') {
      for (const c of this.constructs) {
        const dx = c.x - this.px,
          dz = c.z - this.pz,
          t = dx * this.aimX + dz * this.aimZ,
          lat = Math.abs(dx * this.aimZ - dz * this.aimX);
    ...
    if (mut === 'mass_recoil') {
      this.px -= this.aimX * 0.9;
      this.pz -= this.aimZ * 0.9;
    }
  }
  ...
  private chooseAdaptation(): Adaptation {
    const total = Math.max(1, this.metrics.damage),
      dir = this.directionalDamage / total,
      close = this.closeDamage / total,
      field = this.fieldDamage / total,
      move = this.movementSamples ? this.movementSum / this.movementSamples : 0;
    const r = this.rng.float();
    if (dir > 0.5 && r < 0.68) return 'screening';
    if (close > 0.32 && r < 0.72) return 'repulsor';
    if (move > 0.68 && r < 0.68) return 'intercept';
    if (field > 0.3 && r < 0.72) return 'anchored';
    const pool: Adaptation[] = ['screening', 'repulsor', 'intercept', 'anchored'];
  ...
  private weightedSkillStat(
    st: SkillRuntime,
    pool: (keyof typeof statBase)[]
  ): keyof typeof statBase {
    const weights = new Map<keyof typeof statBase, number>(pool.map((x) => [x, 1]));
    if (st.mutation) {
      const def = skills[st.id].mutations.find((m) => m.id === st.mutation),
        tag = def?.tag ?? '';
      const boost = (names: (keyof typeof statBase)[], m: number) => {
        for (const n of names) if (weights.has(n)) weights.set(n, (weights.get(n) ?? 1) * m);
      };
      ...
      if (['темп', 'движение', 'мобильность'].includes(tag)) boost(['count', 'range', 'crit'], 1.4);
    }
    const entries = [...weights].filter(([, w]) => w > 0);
    let total = entries.reduce((a, [, w]) => a + w, 0),
      r = this.rng.float() * total;
  ...
  snapshot(): Snapshot {
    return {
      ...
      projectiles: this.projectiles.map(
        (p) => ({ id: p.id, x: p.x, z: p.z, vx: p.vx, vz: p.vz, ttl: p.ttl }) as ProjectileSnapshot
      ),
  ...
  canonicalHash() {
    ...
    for (const p of this.pois) parts.push(p.id, p.kind, p.state, p.guardianId, p.x, p.z);
    ...
    parts.push(
      ...
      this.metrics.healingReceived,
      this.metrics.barrierGenerated,
      this.metrics.enemyShotsSpawned,
      this.metrics.enemyShotsHit,
      this.metrics.reactions,
      this.metrics.maxEnemies
    );
