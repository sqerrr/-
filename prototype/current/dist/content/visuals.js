...
const jpg = (id) => ({ normal: `/assets/${id}_normal.jpg`, mutated: `/assets/${id}_mutated.jpg` });
/**
...
export function mutationGlyph(tag, id) {
    const q = (tag + ' ' + id).toLowerCase();
    if (/щит|guard|aegis|защит/.test(q))
        return '⬡';
    // Do not search for bare "лед": it also matches Russian "след" (trail).
    if (/ледян|замор|frost|brittle|whiteout|glacier|spire/.test(q))
        return '❄';
    if (/сеть|chain|circuit|relay|grid|lattice|контур/.test(q))
        return '⟁';
    if (/взрыв|burst|rupture|deton|collapse|singular|shatter/.test(q))
        return '✹';
    if (/маршрут|return|carousel|phoenix|trail|road|след/.test(q))
        return '↺';
    if (/элит|mark|hunter|execution|spot|приоритет/.test(q))
        return '⌖';
    if (/поле|field|mist|crater|storm|туман|облак/.test(q))
        return '◉';
    if (/движ|манёвр|recoil|walker|charge|рывок|ход/.test(q))
        return '➤';
    if (/масса|force|avalanche|gravity|грав/.test(q))
        return '●';
    if (/кров|wound|sanguine|harvest|рана/.test(q))
        return '◆';
    if (/дуг|arc|shock|электр|гроз/.test(q))
        return 'ϟ';
    if (/луч|laser|rail|копь|пронз/.test(q))
        return '↯';
    if (/турел|construct|bastion|battery/.test(q))
        return '▣';
    return '✦';
}
const mutationBranchTone = ['#78d7ff', '#ffb15e', '#d194ff'];
/**
 ...
 */
export function mutationBadge(skill, id) {
    const defs = skills[skill].mutations;
    const byId = new Map(defs.map((d) => [d.id, d]));
    const def = byId.get(id);
    if (!def)
        return { branch: 0, tier: 1, glyph: '?', tone: mutationBranchTone[0], key: `${skill}:missing:${id}` };
    let root = def;
    let tier = def.apotheosis ? 3 : def.parent ? 2 : 1;
    const seen = new Set();
    ...
    const branch = Math.max(0, Math.min(2, index));
    return {
        branch,
        tier,
