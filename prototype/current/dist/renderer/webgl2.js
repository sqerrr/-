...
export class WebGLRenderer {
    ...
    drawWorldShapes(s, aim, presentation) {
        ...
        for (const e of s.entities) {
            ...
            if (e.elite && e.adaptation !== 'none') {
                const ac = e.adaptation === 'screening'
                    ? rgba('#5bbcff', 0.88)
                    : e.adaptation === 'repulsor'
                        ? rgba('#ff7d52', 0.88)
                        : e.adaptation === 'intercept'
    ...
    drawLines(s, aim) {
        ...
        for (const e of s.entities) {
            ...
            if (e.elite) {
                ...
                rect(p.x - bw / 2 + 1, p.y - 91, (bw - 2) * Math.max(0, e.hp / e.maxHp), bh - 2, e.adaptation === 'screening'
                    ? rgba('#56b7ff', 0.95)
                    : e.adaptation === 'repulsor'
                        ? rgba('#ff7d52', 0.95)
                        : e.adaptation === 'intercept'
    ...
    drawSprites(s, presentation) {
        ...
        for (const p of s.projectiles)
            add(p.x, p.z, 18, 18, cellFor.bullet, [1, 1, 1, 0.95]);
        ...
        let o = 0;
        for (const q of list) {
            const cx = q.cell % 8, cy = Math.floor(q.cell / 8), u0 = cx * 0.125, v0 = cy * 0.25, u1 = u0 + 0.125, v1 = v0 + 0.25;
