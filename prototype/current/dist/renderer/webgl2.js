...
export class WebGLRenderer {
    ...
    async loadActorSheet() {
        const loaded = await Promise.all(ACTOR_SOURCES.map(async (src) => {
            const img = new Image();
            img.src = src.url;
            await img.decode();
            return { key: src.key, img };
        }));
        const pad = 8, maxWidth = 2048;
        let penX = pad, penY = pad, shelfHeight = 0, usedWidth = 0;
        const placed = [];
        ...
        const sheetW = Math.max(1, usedWidth + pad), sheetH = Math.max(1, penY + shelfHeight + pad);
        const sheet = document.createElement('canvas');
        sheet.width = sheetW;
        sheet.height = sheetH;
        const ctx = sheet.getContext('2d');
        if (!ctx)
            throw new Error('2D контекст недоступен для сборки листа актёров.');
        ...
        for (const e of cues) {
            ...
            if (e.type === 'skillCast') {
                ...
                if (e.skill === 'ember_lance') {
                    const n = Math.max(1, Math.round(st?.count ?? 1) + (st && st.level >= 7 ? 1 : 0) + Math.min(3, multi)), range = skills.ember_lance.baseRange *
                        (1 + (st?.range ?? 0)) *
                        (st && st.level >= 6 ? 1.16 : st && st.level >= 3 ? 1.07 : 1);
                    for (let i = 0; i < n; i++) {
                        const a = (i - (n - 1) / 2) * 0.085, c = Math.cos(a), q = Math.sin(a), ax = e.aimX * c - e.aimZ * q, az = e.aimX * q + e.aimZ * c;
                        this.fx.push({
                            kind: 'bolt',
                            start: time + i * 0.018,
                            ttl: 0.24,
                ...
                if (e.skill === 'mortar_bloom') {
                    const range = skills.mortar_bloom.baseRange * (1 + (st?.range ?? 0)), d = Math.min(range * 0.72, 12.5), tx = e.x + e.aimX * d, tz = e.z + e.aimZ * d, r = skills.mortar_bloom.baseRadius * Math.sqrt(1 + (st?.coverage ?? 0)) * geom * scale, n = Math.max(1, Math.round(st?.count ?? 1) + Math.min(3, multi));
                    for (let i = 0; i < n; i++) {
                        const a = i ? i * 2.399 : 0, rr = i ? Math.min(1.4, r * 0.38) : 0;
                        this.fx.push({
                            kind: 'ring',
                            start: time + i * 0.045,
                            ttl: 0.82,
    ...
    screenAim(clientX, clientY) {
        const r = this.canvas.getBoundingClientRect();
        const sx = clientX - r.left - r.width * this.centerX, sy = clientY - r.top - r.height * this.centerY;
        const a = sx / Math.max(1, this.isoX), b = sy / Math.max(1, this.isoY);
        const x = (a + b) / 2, z = (b - a) / 2, m = Math.hypot(x, z) || 1;
        return { x: x / m, z: z / m };
    }
    worldToScreen(x, z, s) {
    ...
    combatColor(source, intent, alpha = 1) {
        const q = source.toLowerCase();
        if (q.includes('rail'))
            return rgba('#ff65c8', alpha);
        ...
        if (q.includes('orbit') || q.includes('aegis'))
            return rgba('#77f5d8', alpha);
        if (q.includes('arc') || q.includes('circuit'))
            return rgba('#68cfff', alpha);
        if (q.includes('sentry') || q.includes('battery') || q.includes('grid'))
            return rgba('#5be7c5', alpha);
        if (q.includes('toxic') || q.includes('septic') || q.includes('plague') || q.includes('pestilent'))
            return rgba('#86e46b', alpha);
        if (q.includes('mortar') || q.includes('bombard'))
            return rgba('#ff9a4d', alpha);
        ...
        if (q.includes('tether') || q.includes('gravity') || q.includes('singular'))
            return rgba('#b079ff', alpha);
        if (q.includes('ember'))
            return rgba('#ff9a4d', alpha);
        if (q.includes('repulse'))
            return rgba('#72dcff', alpha);
    ...
    resize() {
        const dpr = Math.max(1, Math.min(2, window.devicePixelRatio || 1)), w = Math.max(1, Math.floor(this.canvas.clientWidth * dpr)), h = Math.max(1, Math.floor(this.canvas.clientHeight * dpr));
    ...
    drawGround(s) {
        const gl = this.gl, p = this.groundProgram;
        gl.useProgram(p);
    ...
    drawWorldShapes(s, aim, presentation) {
        ...
        for (const p of s.projectiles) {
            ...
            if (p.behavior === 'roller') {
                ...
                const phase = p.phase ?? 0, rr = Math.max(0.22, p.radius * (phase === 1 ? 2.6 : 1.7));
        ...
        for (const f of this.combatFx) {
            ...
            if (t < 0 || t > 1 || f.shape.kind !== 'circle')
                continue;
            const c = this.combatColor(f.source, f.intent, (1 - t) * 0.72), q = f.shape;
        ...
        for (const h of presentation.hits) {
            ...
            if (t < 0 || t > 1)
                continue;
            const a = (1 - t) * (0.62 + 0.2 * Math.min(1, h.intensity)), c = h.crit ? rgba('#fff3a8', a) : rgba('#fff8ee', a);
        ...
        for (const p of s.world.pois) {
            if (p.state === 'cleared')
                continue;
            const c = p.kind === 'phenomenon'
                ? rgba('#ff9b4a', 0.72)
                : p.kind === 'catalyst'
                    ? rgba('#c27aff', 0.72)
                    : p.kind === 'resonance'
                        ? rgba('#67d9ff', 0.72)
                        : rgba('#63f0a5', 0.8), pulse = 1 + 0.08 * Math.sin(s.time * 3 + p.id);
        ...
        for (const e of s.entities) {
            ...
            if (e.elite && e.affix === 'shielded') {
                const sx = e.x + Math.cos(e.shieldAngle) * 1.1, sz = e.z + Math.sin(e.shieldAngle) * 1.1, sc = e.shieldState === 'broken' ? '#ff6464' : e.shieldState === 'commit' ? '#ffc261' : '#8bdcff', sa = e.shieldState === 'broken' ? 0.34 : 0.84;
    ...
    drawLines(s, aim) {
        ...
        for (const q of s.pickups) {
            if (q.kind !== 'heal')
                continue;
            const p = this.worldToScreen(q.x, q.z, s), pulse = 0.68 + 0.32 * (0.5 + 0.5 * Math.sin(s.time * 6 + q.id));
        ...
        for (const f of this.combatFx) {
            ...
            if (t < 0 || t > 1 || f.shape.kind === 'circle')
                continue;
            const baseColor = this.combatColor(f.source, f.intent, (1 - t) * 0.72), fill = [
                baseColor[0],
                baseColor[1],
                baseColor[2],
                baseColor[3] * 0.13
        ...
        for (const f of this.combatFx) {
            ...
            if (f.shape.kind === 'circle') {
                const q = f.shape, center = this.worldToScreen(q.x, q.z, s), point = (r, ang) => this.worldToScreen(q.x + Math.cos(ang) * r, q.z + Math.sin(ang) * r, s);
                if (src.includes('frost') || src.includes('glacier') || src.includes('whiteout') || src.includes('spire')) {
                    ...
                    for (let i = 0; i < 6; i++) {
                        const ang = s.time * 0.8 + i * Math.PI / 3, p1 = point(q.radius * 0.28, ang), p2 = point(q.radius * (0.82 + 0.08 * pulse), ang);
                        line(p1.x, p1.y, p2.x, p2.y, 1.8, c);
                    }
                }
                else if (src.includes('orbit') || src.includes('aegis')) {
                    // Four tangent blade marks circle the perimeter.
                    for (let i = 0; i < 4; i++) {
                        const ang = -s.time * 2.4 + i * Math.PI / 2, p = point(q.radius, ang), p2 = point(q.radius, ang + 0.16);
                        line(p.x, p.y, p2.x, p2.y, 4.2, c);
                    }
                }
                    ...
                    const rr = Math.max(10, q.radius * this.isoX * 0.56);
                    line(center.x - rr, center.y, center.x + rr, center.y, 1.4, c);
                    line(center.x, center.y - rr * 0.55, center.x, center.y + rr * 0.55, 1.4, c);
                    line(center.x, center.y - 80 * (1 - t) - 18, center.x, center.y - 8, 2.6, c);
                }
                    ...
                    for (let i = 0; i < 5; i++) {
                        const ang = i * 2.17 + s.time * (i % 2 ? 0.28 : -0.22), r = q.radius * (0.3 + 0.11 * i), p = point(r, ang), rr = 3 + 2 * Math.sin(s.time * 3 + i);
                        line(p.x - rr, p.y, p.x + rr, p.y, Math.max(1, 2 - t), c);
                    }
                }
                    ...
                    for (let i = 0; i < 6; i++) {
                        const ang = i * Math.PI / 3 + s.time * 0.18, po = point(q.radius * (0.82 - 0.08 * pulse), ang), pi = point(q.radius * 0.34, ang);
                        line(po.x, po.y, pi.x, pi.y, 2.3, c);
                    }
                }
                    ...
                    for (let i = 0; i < 3; i++) {
                        const ang = -s.time * 4 + i * Math.PI * 2 / 3, p1 = point(q.radius * 0.35, ang), p2 = point(q.radius * 0.9, ang + 0.34);
                        line(p1.x, p1.y, p2.x, p2.y, 3, c);
                    }
                }
            }
                ...
                if (src.includes('rail')) {
                    ...
                    line(end.x - 7, end.y, end.x + 7, end.y, 2, c);
                    line(end.x, end.y - 5, end.x, end.y + 5, 2, c);
                }
                    ...
                    for (let i = 1; i < 5; i++) {
                        const u = (i / 5 + s.time * 0.7) % 1, x = start.x + (end.x - start.x) * u, y = start.y + (end.y - start.y) * u;
                        line(x - 3, y - 3, x + 3, y + 3, 2, c);
                    }
                }
                    ...
                    for (let i = 0; i < 5; i++) {
                        const u = (i + 0.25) / 5, v = (i + 0.65) / 5;
                        line(start.x + (end.x - start.x) * u, start.y + (end.y - start.y) * u, start.x + (end.x - start.x) * v, start.y + (end.y - start.y) * v, 1.7, c);
                    }
                }
                    ...
                    for (let i = 1; i < 5; i++) {
                        const u = i / 5, x = start.x + (end.x - start.x) * u, y = start.y + (end.y - start.y) * u, dx = end.x - start.x, dy = end.y - start.y, ll = Math.hypot(dx, dy) || 1, nx = -dy / ll, ny = dx / ll;
                        line(x - nx * 5, y - ny * 5, x + nx * 5, y + ny * 5, 2.4, c);
                    }
                }
            }
            else if (f.shape.kind === 'sector') {
                ...
                if (src.includes('cleaver') || src.includes('harvest') || src.includes('wound')) {
                    ...
                    for (let k = 0; k < 2; k++) {
                        const u = Math.min(1, t * 1.35 + k * 0.24), ang = base - q.halfAngle + 2 * q.halfAngle * u, p = this.worldToScreen(q.x + Math.cos(ang) * q.radius, q.z + Math.sin(ang) * q.radius, s);
                        line(center.x, center.y, p.x, p.y, 3.4 - k, c);
                    }
                }
            }
        }
        ...
        for (const e of s.entities) {
            ...
            if (e.elite && e.affix === 'temporal') {
                ...
                line(a.x - Math.cos(ang) * r, a.y - 42 - Math.sin(ang) * r * 0.45, a.x + Math.cos(ang) * r, a.y - 42 + Math.sin(ang) * r * 0.45, 2.2, rgba('#b8a4ff', 0.62));
                line(a.x - Math.cos(ang + Math.PI / 2) * r, a.y - 42 - Math.sin(ang + Math.PI / 2) * r * 0.45, a.x + Math.cos(ang + Math.PI / 2) * r, a.y - 42 + Math.sin(ang + Math.PI / 2) * r * 0.45, 2.2, rgba('#7c6dff', 0.48));
            }
            if (e.orderActive && e.squadTask !== 'none') {
                const a = this.worldToScreen(e.x, e.z, s), b = this.worldToScreen(e.orderX, e.orderZ, s), tc = e.squadTask === 'flank' ? '#c58cff' : e.squadTask === 'intercept' ? '#ffb45f' : e.squadTask === 'hold' ? '#78d4ff' : '#8ee28a';
                line(a.x, a.y - 14, b.x, b.y - 8, 1.2, rgba(tc, 0.24));
            }
            if (e.elite && e.affix === 'shielded') {
                const a = this.worldToScreen(e.x, e.z, s), ang = e.shieldAngle, rr = 2.15, pA = this.worldToScreen(e.x + Math.cos(ang - 0.78) * rr, e.z + Math.sin(ang - 0.78) * rr, s), pB = this.worldToScreen(e.x + Math.cos(ang + 0.78) * rr, e.z + Math.sin(ang + 0.78) * rr, s), front = this.worldToScreen(e.x + Math.cos(ang) * rr, e.z + Math.sin(ang) * rr, s);
                const shieldColor = e.shieldState === 'broken' ? '#ff6268' : e.shieldState === 'commit' ? '#ffc05c' : '#7dd9ff', alpha = e.shieldState === 'broken' ? 0.28 : 0.84;
            ...
            if (e.elite) {
                ...
                rect(p.x - bw / 2 + 1, p.y - 91, (bw - 2) * Math.max(0, e.hp / e.maxHp), bh - 2, rgba('#7fe46f', 0.95));
                if (e.affix === 'shielded') {
        ...
        for (const f of this.fx) {
            ...
            if (f.kind === 'beam') {
                ...
                const q = 1 - Math.pow(1 - t, 2), q0 = Math.max(0, q - 0.12), a = this.worldToScreen(f.x1 + (f.x2 - f.x1) * q0, f.z1 + (f.z2 - f.z1) * q0, s), b = this.worldToScreen(f.x1 + (f.x2 - f.x1) * q, f.z1 + (f.z2 - f.z1) * q, s);
                ...
                let prev = null;
                for (let i = 0; i <= steps; i++) {
                    const q = i / steps, ang = base - 1.05 + 2.1 * q * progress;
    ...
    drawSprites(s, presentation) {
        const list = [];
        const push = (x, z, w, h, tex, uv, tint, flip) => {
            const p = this.worldToScreen(x, z, s);
            list.push({ x, z, w, h, tex, ...uv, tint, flip, sort: p.y });
        };
        const add = (x, z, w, h, cell, tint = [1, 1, 1, 1], flip = 0) => {
            const cx = cell % 8, cy = Math.floor(cell / 8);
            push(x, z, w, h, TEX_ATLAS, { u0: cx * 0.125, v0: cy * 0.25, u1: cx * 0.125 + 0.125, v1: cy * 0.25 + 0.25 }, tint, flip);
        };
        const addActor = (x, z, w, h, key, tint = [1, 1, 1, 1], flip = 0) => {
            const uv = this.actorFrames.get(key);
            if (uv)
                push(x, z, w, h, TEX_ACTORS, uv, tint, flip);
        };
        const entityVisual = (e, pulse = 1) => {
            ...
            const actor = e.elite
                ? this.actorFrames.has('elite_' + chassis)
                    ...
                    : 'elite_warden'
                : null;
            const largeElite = e.chassis === 'bulwark' ||
                e.chassis === 'architect' ||
                e.chassis === 'harvester' ||
                e.chassis === 'broodmaker' ||
                e.boss;
            ...
            let tint = [1, 1, 1, 1];
            if (e.elite && e.affix === 'crowned')
                tint = [1.16, 1.08, 0.72, 1];
            else if (e.elite && e.affix === 'temporal')
                tint = [0.95, 0.9, 1.18, 1];
            else if (e.elite && e.affix === 'vanguard')
                tint = [1.12, 0.95, 0.78, 1];
            else if (e.elite && e.affix === 'brood')
                tint = [1.12, 0.82, 0.92, 1];
            if (e.buffed)
                tint = [1.15, 0.96, 0.76, 1];
            if (e.revived)
                tint = [0.86, 1.12, 1.12, 1];
        ...
        const addVisual = (x, z, w, h, v, tint, flip) => {
            if (v.actor)
                addActor(x, z, w, h, v.actor, tint, flip);
            else
                add(x, z, w, h, v.cell, tint, flip);
        };
        for (const p of s.pickups) {
        ...
        for (const c of s.constructs)
            add(c.x, c.z, 58, 68, cellFor.sentry, [1, 1, 1, 0.95]);
        const hitById = new Map(presentation.hits.map((h) => [h.entity, h]));
        for (const e of s.entities) {
            const bob = Math.sin(s.time * (e.elite ? 2.2 : 3.4) + e.id * 0.71), pulse = 1 + (e.elite ? 0.035 : 0.018) * bob, v = entityVisual(e, pulse), hit = hitById.get(e.id);
            ...
            if (e.elite && e.echoPhase && e.echoPhase !== 'none') {
                if (e.echoPhase === 'tell') {
                    ...
                    w *= charge;
                    h *= 0.92;
                    tint = [Math.min(1.5, tint[0] * 1.12), Math.min(1.5, tint[1] * 1.12), Math.min(1.5, tint[2] * 1.12), tint[3]];
                }
                    ...
                    w *= 1.04;
                    h *= 0.86;
                    tint = [tint[0] * 0.78, tint[1] * 0.82, tint[2] * 0.9, tint[3]];
                }
            }
            if (hit) {
                const t = Math.max(0, Math.min(1, (s.time - hit.start) / hit.ttl)), snap = 1 - t, kick = (e.elite ? 0.1 : 0.18) * hit.intensity * snap;
                x += hit.dirX * kick;
                z += hit.dirZ * kick;
        ...
        for (const d of presentation.deaths) {
            const t = Math.max(0, Math.min(1, (s.time - d.start) / d.ttl)), ease = 1 - Math.pow(1 - t, 2), v = entityVisual(d.actor, 1), travel = d.actor.elite ? 0.42 : 0.72, x = d.x + d.dirX * travel * ease, z = d.z + d.dirZ * travel * ease, alpha = Math.pow(1 - t, 0.65), w = v.w * (1 + 0.18 * ease), h = v.h * Math.max(0.2, 1 - 0.76 * ease), warm = d.actor.elite ? 0.86 : 0.66;
        ...
        while (runStart < list.length) {
            ...
            while (runEnd < list.length && list[runEnd].tex === tex)
                runEnd++;
            gl.bindTexture(gl.TEXTURE_2D, tex === TEX_ACTORS ? this.actorTex : this.atlas);
            gl.bufferData(gl.ARRAY_BUFFER, this.spriteData.subarray(runStart * stride, runEnd * stride), gl.DYNAMIC_DRAW);
    ...
    pushLine(v, x1, y1, x2, y2, w, c) {
        const dx = x2 - x1, dy = y2 - y1, l = Math.hypot(dx, dy) || 1, nx = (-dy / l) * w * 0.5, ny = (dx / l) * w * 0.5;
        const pts = [
            [x1 + nx, y1 + ny],
            [x1 - nx, y1 - ny],
            [x2 + nx, y2 + ny],
    ...
    makeSpriteVao() {
        const gl = this.gl, vao = gl.createVertexArray(), quad = gl.createBuffer(), inst = gl.createBuffer();
    ...
    instAttrib(loc, size, stride, offset) {
        const gl = this.gl;
    ...
    async loadTexture(url, repeat = false) {
        const img = new Image();
        img.src = url;
        await img.decode();
        const gl = this.gl, t = gl.createTexture();
        ...
        gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_MAG_FILTER, gl.LINEAR);
        gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_S, repeat ? gl.MIRRORED_REPEAT : gl.CLAMP_TO_EDGE);
        gl.texParameteri(gl.TEXTURE_2D, gl.TEXTURE_WRAP_T, repeat ? gl.MIRRORED_REPEAT : gl.CLAMP_TO_EDGE);
        gl.pixelStorei(gl.UNPACK_PREMULTIPLY_ALPHA_WEBGL, 0);
