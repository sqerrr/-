import { activeSkillOrder, catalysts, doctrines, mutationDef, rarityColor, rarityName, skills } from '../content/definitions.js';
...
function skillVisualIcon(id, mutated = false) {
...
let startingSkill = activeSkillOrder.includes(params.get('start'))
    ? params.get('start')
    : 'cleaver';
...
let eliteAlertToken = 0, rareAlertUntil = 0;
...
function modalState() {
    const w = $('choice'), cs = getComputedStyle(w);
    return {
        hidden: w.hidden,
        display: cs.display,
...
function updateDebugState() {
    const s = sim.snapshot(), state = `${s.choiceSerial}|${!!s.rewardOffers}|${!!s.mutationOffer}|${modalState().hidden}|${choiceLocked}`;
...
window.addEventListener('error', (e) => dbg('WINDOW ERROR', {
    message: e.message,
    file: e.filename,
    line: e.lineno,
...
function screenMove() {
    ...
    if (!sx && !sy)
        return { x: 0, z: 0 };
    const x = sx + sy, z = -sx + sy, m = Math.hypot(x, z) || 1;
    return { x: x / m, z: z / m };
}
function keyHandled(k) {
    return [
        'w',
        'a',
        's',
...
function togglePause() {
    ...
    if (planning || s.player.hp <= 0 || s.finished || s.rewardOffers || s.mutationOffer)
        return;
    paused = !paused;
...
function togglePlanning() {
    const s = sim.snapshot();
    if (s.player.hp <= 0 || s.finished || s.rewardOffers || s.mutationOffer)
        return;
    planning = !planning;
...
function restart() {
    ...
    pushLog(`${runMode === 'clean' ? 'Чистый ран' : 'Showcase'}: исследуйте отмеченные узлы карты; Хранитель приходит в финале.`);
}
const chassisName = {
    marshal: 'Маршал',
    hunter: 'Хищник',
...
function eventText(e) {
    ...
    if (e.type === 'EliteOrder') {
        const m = {
            ...
            prism: 'Призма запомнила источник. Повторять его подряд невыгодно — смена источника вскрывает защиту.',
    ...
    if (e.type === 'RivalCast')
        return `Элита применила Echo отвергнутого: ${skills[e.skill].name}.`;
    ...
    if (e.type === 'RelicAppeared')
        return `На поле появилась находка: ${e.name}.`;
...
function showEliteAlert(title, body, duration = 3400, rare = false) {
    const now = performance.now();
    // A frequent Echo tell must never erase a rare structural event before the player can read it.
    if (!rare && now < rareAlertUntil)
        return;
    if (rare)
        rareAlertUntil = Math.max(rareAlertUntil, now + duration);
    const token = ++eliteAlertToken, el = $('eliteAlert');
...
function pushEvents(events) {
    for (const e of events) {
        ...
        if (a) {
            if (e.type === 'EliteEchoPhase')
                showEliteAlert(a[0], a[1], 1150, false);
            else if (e.type === 'RareEvent')
                showEliteAlert(a[0], a[1], 2600, true);
            else
                showEliteAlert(...a);
        }
        if (e.type === 'DamageResolved') {
            const now = sim.time, prev = [...combatFloats]
                .reverse()
...
function updateChain(s) {
    ...
    if (sig !== chainSignature) {
        ...
        s.chain.slots.forEach((id, i) => {
            ...
            if (id) {
                const st = rt.get(id), md = st.mutation ? mutationDef(id, st.mutation) : null, md2 = st.mutationUpgrade ? mutationDef(id, st.mutationUpgrade) : null;
...
function attachPlannerDnD(el) {
    ...
    el.addEventListener('drop', (e) => {
        ...
        const src = e.dataTransfer?.getData('text/plain') ?? '', dst = el.dataset.drag ?? '';
        const a = src.split(':'), b = dst.split(':');
        ...
        if (a[0] === 'skill')
            ...
            ok = sim.swapCatalystLocations(a[1], Number(a[2]), b[1], Number(b[2]));
        if (ok) {
            chainSignature = '';
            plannerSignature = '';
...
function plannerSkillNode(id, zone, idx, s) {
    const el = document.createElement('div');
    el.className = 'pnode skill' + (id ? '' : ' empty');
    el.draggable = true;
    el.dataset.drag = `skill:${zone}:${idx}`;
    if (id) {
        const st = s.skills.find((x) => x.id === id), d = skills[id], md = st.mutation ? mutationDef(id, st.mutation) : null, md2 = st.mutationUpgrade ? mutationDef(id, st.mutationUpgrade) : null;
        ...
        el.innerHTML = '<div>пустой<br>слот</div>';
    attachPlannerDnD(el);
    return el;
}
function plannerCatNode(id, zone, idx, s) {
    const el = document.createElement('div');
    ...
    if (id) {
        ...
        el.innerHTML = '<div>пустой<br>slot</div>';
    attachPlannerDnD(el);
    return el;
}
function updatePlanner(s) {
    if (!planning)
        return;
    const sig = s.chain.slots.join('|') +
        '#' +
...
function drawMinimap(s) {
    ...
    const tx = (x) => pad + ((x - s.world.minX) / (s.world.maxX - s.world.minX)) * (w - pad * 2), ty = (z) => pad + ((z - s.world.minZ) / (s.world.maxZ - s.world.minZ)) * (h - pad * 2);
 ...
 */
function drawHeldItems(ctx, s) {
    if (!s.heldItems.length)
        return;
    const counts = new Map();
    for (const id of s.heldItems)
        counts.set(id, (counts.get(id) ?? 0) + 1);
    const entries = [...counts.entries()];
    const w = 46, h = 18, gap = 4;
    let x = 18;
    ...
    for (const [id, n] of entries) {
        ...
        x += w + gap;
        if (x > ctx.canvas.width / (window.devicePixelRatio || 1) - w)
            break;
    }
    ctx.restore();
}
...
function drawRefusalRow(ctx, icons, titles, kinds, cx, cy, tint) {
    const n = Math.min(icons.length, 6);
    if (!n)
        return;
    const size = 18, rowH = 20, pad = 5;
    ctx.save();
    ctx.textBaseline = 'middle';
    ctx.font = '700 11px system-ui';
    let widest = 0;
    for (let i = 0; i < n; i++)
        widest = Math.max(widest, ctx.measureText(titles[i] ?? '').width);
    const boxW = size + pad + widest + pad * 2;
    // Stacked upwards so the newest row never covers the elite's own label.
    for (let i = 0; i < n; i++) {
        const y = cy - (n - 1 - i) * rowH, x = cx - boxW / 2, kindTint = refusalKindTint[kinds[i]] ?? tint;
        ...
        const token = icons[i] ?? '', img = token.indexOf('/') >= 0 ? hudImage(token) : null;
        if (img)
            ctx.drawImage(img, x + pad, y - size / 2, size, size);
        else {
...
function drawCombatHud(s) {
    ...
    for (const e of s.entities) {
        ...
        if (e.elite && e.affix === 'shielded') {
            ...
            ctx.fillStyle = '#05080bd9';
            ctx.fillRect(p.x - bw / 2, sy, bw, 4);
            ctx.fillStyle = st;
            ctx.fillRect(p.x - bw / 2 + 1, sy + 1, (bw - 2) * Math.max(0, Math.min(1, e.shieldStability / 100)), 2);
        }
        if (e.elite) {
            ...
            drawRefusalRow(ctx, e.refusalIcons ?? [], e.refusalTitles ?? [], e.refusalKinds ?? [], p.x, y - 30, tint);
        }
    }
    const now = s.time;
...
function updateThreatPanel(s) {
    const box = $('threatPanel'), boss = s.entities.find((e) => e.boss), elites = s.entities.filter((e) => e.elite && !e.boss);
    const e = boss ??
        elites.sort((a, b) => Math.hypot(a.x - s.player.x, a.z - s.player.z) -
...
function updateUi(s) {
    const mm = Math.floor(s.time / 60), ss = Math.floor(s.time % 60)
        .toString()
        .padStart(2, '0'), rm = Math.floor(s.runDuration / 60), rs = Math.floor(s.runDuration % 60)
        .toString()
        .padStart(2, '0');
    ...
    const cleared = s.world.pois.filter((p) => p.state === 'cleared').length, remaining = Math.max(0, s.runDuration * 0.875 - s.time), bossSupport = s.entities.filter((e) => e.elite && !e.boss && e.guardianPoi !== 0).length;
    $('objective').textContent = s.world.bossSpawned
        ? s.world.bossDefeated
            ? 'Хранитель уничтожен'
            : `ЦЕЛЬ: ХРАНИТЕЛЬ${bossSupport ? ` · поддержка стражей ${bossSupport}` : ''}`
        : `Исследование ${cleared}/${s.world.pois.length} · активируй 4+ узла, чтобы ослабить финальную поддержку · ${Math.floor(remaining / 60)}:${Math.floor(remaining % 60)
            .toString()
            .padStart(2, '0')}`;
...
function finishChoiceAction(label, action) {
    ...
    choiceLocked = false;
    syncChoiceUI(after);
    setTimeout(() => dbg('CHOICE +400ms', { sim: snapChoice(sim.snapshot()), modal: modalState() }), 400);
}
function refreshChoiceAction(label, action) {
    if (choiceLocked)
        return;
    let ok = false;
    try {
...
function offerCategory(o) {
    if (o.kind === 'skill_add' || o.kind === 'skill_swap')
        return 'phenomenon';
    if (o.kind === 'catalyst_add')
        return 'catalyst';
    if (o.kind === 'item_grant' || o.kind === 'elite')
        return 'item';
    if (o.kind === 'resonance')
        return 'resonance';
    if (o.kind === 'doctrine')
        return 'doctrine';
    if (o.kind === 'mutation_target')
        return 'mutation';
    return 'global';
}
function offerIcon(o) {
    if (o.skill)
        return `<img class="choice-icon-img" src="${skillVisualIcon(o.skill, false)}" alt="">`;
    if (o.doctrine) {
        const d = doctrines[o.doctrine];
        return `<span class="choice-glyph" style="color:${d.color}">${esc(d.glyph)}</span>`;
    }
    if (o.catalyst)
        return `<span class="choice-glyph catalyst-glyph" style="color:${catalysts[o.catalyst].color}">${esc(catalystGlyph[o.catalyst])}</span>`;
    if (o.item) {
        const d = itemDefs[o.item];
        return `<span class="choice-glyph item-glyph" style="color:${itemCategoryColor[d.category]}">${esc(itemGlyph[o.item])}</span>`;
    }
    return `<span class="choice-glyph">◆</span>`;
}
function shortPromise(text) {
    const first = text.split(/(?<=[.!?])\s/)[0] || text;
    return first.length > 112 ? first.slice(0, 109) + '…' : first;
}
function categoryLabel(cat) {
    return { phenomenon: 'PHENOMENON', catalyst: 'CATALYST', item: 'ITEM', resonance: 'RESONANCE', doctrine: 'DOCTRINE', mutation: 'MUTATION', global: 'CORE' }[cat] ?? cat.toUpperCase();
}
function syncChoiceUI(s, force = false) {
    ...
    if (s.mutationOffer) {
        ...
        m.choices.forEach((id, i) => {
            const d = mutationDef(m.skill, id), badge = mutationBadge(m.skill, id), card = document.createElement('div');
            ...
            const choose = () => finishChoiceAction(`mutation:${m.skill}:${i}:${id}`, () => sim.chooseMutation(i));
            card.addEventListener('click', (e) => {
                if (e.target.closest('.refuse'))
                    return;
                choose();
            });
            ...
            card.querySelector('.refuse').addEventListener('click', (e) => {
                e.stopPropagation();
                if (m.refusalAvailable)
                    refreshChoiceAction(`refuse:${i}`, () => sim.refuseMutation(i));
            });
            cards.append(card);
        });
    }
        ...
        s.rewardOffers.forEach((o, i) => {
            const card = document.createElement('div'), rar = o.rarity;
...
async function start() {
    ...
    try {
        ...
        $('loading').classList.add('hidden');
        pushLog('v0.11: XP развивает Doctrines; Phenomena, Catalysts, Items и Mutation Core приходят из отдельных каналов.');
        pushLog(runMode === 'clean'
