...
for (const seed of seeds) {
    ...
    for (let i = 0; i < 480 * 30; i++) {
        ...
        const t = s.entities.filter(e => e.elite).sort((a, b) => Math.hypot(a.x - s.player.x, a.z - s.player.z) - Math.hypot(b.x - s.player.x, b.z - s.player.z))[0] ?? s.entities[0];
        let ax = 1, az = 0;
        if (t) {
        ...
        const a = s.time / 5.5;
        sim.step({ moveX: Math.cos(a) * .45, moveZ: Math.sin(a * .77) * .45, aimX: ax, aimZ: az });
        while (sim.hasChoice) {
...
const out = [...rows].map(([t, r]) => ({ t, enemies: +avg(r.enemies).toFixed(1), elites: +avg(r.elites).toFixed(1), enemyProjectiles: +avg(r.shots).toFixed(1), hp: +avg(r.hp).toFixed(0), kills: +avg(r.kills).toFixed(1), samples: r.enemies.length }));
