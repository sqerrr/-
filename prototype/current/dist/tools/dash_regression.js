...
function assert(ok, what) {
    if (!ok)
        throw new Error('dash regression: ' + what);
}
function fresh() {
    const sim = new Simulation({ seed: 4242, hz: 60, mode: 'clean' });
    return { sim, s: sim };
}
const travelled = (dash) => {
    const { sim, s } = fresh();
    const x0 = s.px;
    for (let i = 0; i < 12; i++)
...
const walk = travelled(false), lunge = travelled(true);
assert(lunge > walk * 3, 'a dash should cover far more ground than walking: ' + lunge + ' vs ' + walk);
const { sim, s } = fresh();
...
while (s.time < s.dashIFramesUntil)
    sim.step({ moveX: 1, moveZ: 0, aimX: 1, aimZ: 0 });
...
assert(exposed > Simulation.DASH_IFRAMES, 'between two windows the hero must spend longer exposed than protected');
while (s.time < s.dashReadyAt)
    sim.step({ moveX: 1, moveZ: 0, aimX: 1, aimZ: 0 });
sim.step({ moveX: 1, moveZ: 0, aimX: 1, aimZ: 0, dash: true });
...
const tracked = new Simulation({ seed: 4243, hz: 60, benchmark: true });
tracked.spawnElite();
const trackedElite = tracked.ents.find((e) => e.kind === 'elite');
