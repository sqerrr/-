...
function assert(ok, message) {
    if (!ok)
        throw new Error('projectile-regression: ' + message);
}
function projectile(s, faction, x, z, vx, vz, damage = 40) {
    s.spawnProjectile({
        x,
        z,
        vx,
...
const heroSim = new Simulation({ seed: 12345, hz: 60, benchmark: true });
...
const coverSim = new Simulation({ seed: 777, hz: 60, benchmark: true });
const soft = coverSim.obstacles.find((o) => o.destructible);
const hard = coverSim.obstacles.find((o) => !o.destructible);
assert(soft && hard, 'arena must contain both cover classes');
coverSim.obstacles = [soft];
coverSim.buildObstacleGrid();
projectile(coverSim, 'hero', soft.x - soft.radius - 1.2, soft.z, 60, 0, soft.maxHp + 20);
for (let i = 0; i < 6; i++)
    coverSim.updateProjectiles();
assert(coverSim.obstacles.length === 0, 'destructible cover survived lethal damage');
const hardSim = new Simulation({ seed: 777, hz: 60, benchmark: true });
const hardOnly = hardSim.obstacles.find((o) => !o.destructible);
...
for (let i = 0; i < 6; i++)
    hardSim.updateProjectiles();
assert(hardSim.obstacles.length === 1, 'permanent cover was destroyed');
function rivalLoss(guard) {
    const sim = new Simulation({ seed: 4242, hz: 60, benchmark: true });
    sim.px = 0;
    sim.pz = 0;
    sim.barrier = 0;
    if (guard) {
