...
function assert(c, m) { if (!c)
    throw new Error('v0.11 regression: ' + m); }
...
for (const a of apotheoses)
    assert(classSource.includes(`'${a.id}'`) || classSource.includes(`\"${a.id}\"`), `${a.skill}/${a.id} is catalogue-only: no runtime branch`);
...
for (const skill of activeSkillOrder) {
    const sim = new Simulation({ seed: 91000 + activeSkillOrder.indexOf(skill), hz: 60, benchmark: true, mode: 'clean' });
    sim.spawnElite();
    const owner = sim.ents.find((e) => e.kind === 'elite');
    assert(owner, `${skill}: failed to spawn elite owner`);
    sim.ents = [owner];
    ...
    assert(sim.events.some((e) => e.type === 'EliteEchoPhase' && e.phase === 'tell' && e.skill === skill), `${skill}: no Echo tell`);
    const shapes = sim.events.filter((e) => e.type === 'CombatShape' && String(e.source).startsWith(`echo_${skill}_tell`));
    assert(shapes.length > 0, `${skill}: no authored Echo telegraph geometry`);
    echoShapes += shapes.length;
    const q = sim.eliteEchoes.get(owner.id);
    assert(q, `${skill}: missing Echo state`);
    sim.events = [];
    sim.tick = Math.ceil(q.until * sim.hz);
    sim.updateEliteEchoes();
    assert(sim.events.some((e) => e.type === 'EliteEchoPhase' && e.phase === 'active'), `${skill}: no active Echo phase`);
    assert(!sim.events.some((e) => e.type === 'SkillActivated'), `${skill}: Echo reused player cast dispatcher`);
}
// Shielded elites must expose an authored melee/Force break window rather than infinitely tracking.
const shield = new Simulation({ seed: 99001, hz: 60, benchmark: true });
shield.spawnElite();
const e = shield.ents.find((x) => x.kind === 'elite');
assert(e, 'shield test: no elite');
e.affix = 'shielded';
e.shieldState = 'guard';
e.shieldStability = 100;
e.shieldAngle = 0;
e.hp = e.maxHp = 1e9;
shield.doctrines.force = 12;
shield.events = [];
shield.damage(e, 3000, 'cleaver', true, e.x + 5, e.z);
assert(e.shieldState === 'broken', 'Force/melee failed to break shield state');
assert(shield.events.some((x) => x.type === 'RareEvent' && x.title === 'ЩИТ СЛОМАН'), 'shield break lacks rare/readable event');
// Snapshot must carry physical-behaviour and body-language state to the renderer.
const snap = shield.snapshot();
const se = snap.entities.find((x) => x.id === e.id);
assert(se?.shieldState === 'broken', 'snapshot lost shield state');
