...
function assert(ok, msg) {
    if (!ok)
        throw new Error('geometry-regression: ' + msg);
}
const idle = { moveX: 0, moveZ: 0, aimX: 1, aimZ: 0 };
function make(seed) {
    return new Simulation({ seed, hz: 60 });
}
...
assert(walked.closest > walked.limit - 0.05, `walked into cover: reached ${walked.closest.toFixed(2)} against ${walked.limit.toFixed(2)}`);
// Reaching the surface is required too, otherwise the check would pass on a hero
// that simply never arrived.
assert(walked.closest < walked.limit + 0.6, `never reached the cover: stopped at ${walked.closest.toFixed(2)}`);
const dashed = driveInto(true);
assert(dashed.closest > dashed.limit - 0.05, `dashed through cover: reached ${dashed.closest.toFixed(2)} against ${dashed.limit.toFixed(2)}`);
...
for (let i = 0; i < 2400; i++) {
    ...
    const heroOverlap = Math.max(0, ...sim.obstacles.map((o) => o.radius + 0.42 - Math.hypot(sim.px - o.x, sim.pz - o.z)));
