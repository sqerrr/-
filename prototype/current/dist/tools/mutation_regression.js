...
const skill = 'frost_ring';
const sim = new Simulation({ seed: 301991, hz: 60, benchmark: true });
...
sim.generateMutationTargetOffers();
assert(sim.chooseReward(0), 'failed Tier II target');
...
sim.generateMutationTargetOffers();
assert(sim.chooseReward(0), 'failed Tier III target');
offer = sim.mutationOffer;
...
assert(sim.events.some((e) => e.type === 'RareEvent' && e.title === 'АПОФЕОЗ'), 'Tier III must emit a rare visual event');
sim.skillReserve[0] = 'rail_spear';
sim.skillsRuntime.set('rail_spear', sim.newSkill('rail_spear'));
